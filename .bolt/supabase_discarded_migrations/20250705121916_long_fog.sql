/*
  ...
  This migration ensures that all foreign key constraints in the data_structures table
  have the correct names and references, which is critical for proper joins in the application.

  ## Changes
  1. Drop all existing foreign key constraints to avoid conflicts
  2. Re-add constraints with standardized names that match what the application expects
  3. Force schema cache refresh multiple times to ensure PostgREST picks up the changes
  4. Verify that constraints are correctly set up
*/
