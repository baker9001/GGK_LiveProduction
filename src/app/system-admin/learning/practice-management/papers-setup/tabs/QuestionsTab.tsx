...
import {
  ExamSimulation,
  type QAReviewQuestionResult,
  type QAReviewResultPayload,
  type QAReviewSummary
} from '../../questions-setup/components/ExamSimulation';
import {
...
function QuestionsTabInner({
  ...
  const handleStartSimulation = () => {
    try {
      const derivedTotalMarks = typeof paperMetadata?.total_marks === 'number'
        ? paperMetadata.total_marks
        : Number(paperMetadata?.total_marks) || questions.reduce((sum, q) => sum + (q.marks || 0), 0);

      const subjectForSimulation = (paperMetadata?.subject || paperTitleForMetadata || 'IGCSE Practice Paper').toString();
      const simulationPaper = buildSimulationPaper({
        questions,
        paperTitle: paperTitleForMetadata || paperMetadata?.paper_title || 'Preview Paper',
        totalMarks: derivedTotalMarks,
        paperCode: paperCodeForMetadata || paperMetadata?.paper_code || 'qa-preview',
        subject: subjectForSimulation,
        duration: paperMetadata?.paper_duration ? String(paperMetadata.paper_duration) : null,
        ...
        validationConfig: {
          ...
          subject_specific_rules: {
            chemistry: subjectForSimulation.toLowerCase().includes('chemistry'),
            physics: subjectForSimulation.toLowerCase().includes('physics'),
            biology: subjectForSimulation.toLowerCase().includes('biology'),
            mathematics: subjectForSimulation.toLowerCase().includes('math')
          }
        }
      });
  ...
  const handleSimulationExit = async (result?: QAReviewResultPayload) => {
    ...
    if (result) {
      const simulationResult: SimulationResult = {
        ...
        overallScore: result.summary?.percentage ?? result.score,
        timeSpent: result.timeElapsed,
        answeredCount: result.summary?.answeredQuestions ?? result.answeredCount,
        totalQuestions: result.summary?.totalQuestions ?? result.totalQuestions,
