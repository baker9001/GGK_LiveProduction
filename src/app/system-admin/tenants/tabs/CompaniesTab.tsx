  ...
  const deleteMutation = useMutation(
    async (companies: Company[]) => {
      ...
      for (const company of companies) {
        await deleteFileFromStorage('company-logos', company.logo);
      }
  ...
  const columns = [
    {
      ...
      cell: (row: Company) => (
        <div className="w-10 h-10 flex items-center justify-center">
          {row.logo ? (
            <img
              src={getPublicUrl('company-logos', row.logo)}
              alt={`${row.name} logo`}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            ...
            <ImageUpload
              ...
              value={editingCompany?.logo}
              publicUrl={editingCompany?.logo ? getPublicUrl('company-logos', editingCompany.logo) : null}
              onChange={(path) => {
