...
interface EnhancedComplexQuestionDisplayProps {
...
const EnhancedComplexQuestionDisplay: React.FC<EnhancedComplexQuestionDisplayProps> = ({
  question,
  value,
  onChange,
  context = 'practice',
  ...
  // Get display configuration for the context
  const displayConfig = useMemo(() => createDisplayConfig(context), [context]);

  const togglePart = (partId: string) => {
    if (!displayConfig.element.allowPartCollapse) return;
  ...
  const handlePartAnswerChange = (partId: string, partLabel: string, answer: any, subparts?: ComplexSubpartAnswer[]) => {
    if (!onChange || !displayConfig.element.allowAnswerInput) return;
  ...
  const renderAttachments = (attachments?: any[]) => {
    if (!attachments || attachments.length === 0 || !displayConfig.element.showAttachments) return null;
  ...
  const renderMetadata = (element: any) => {
    if (!displayConfig.element.showMetadata) return null;

    return (
      <div className="mt-2 p-2 bg-gray-50 dark:bg-gray-900/30 rounded border border-gray-200 dark:border-gray-700 text-xs space-y-1">
        <div className="font-medium text-gray-700 dark:text-gray-300 mb-1">Metadata:</div>

        {displayConfig.element.showContainerFlags && (
          <>
            {element.is_container !== undefined && (
              <div className="flex items-center gap-2">
                <span className="text-gray-600 dark:text-gray-400">is_container:</span>
                <span className={cn(
                  "px-1.5 py-0.5 rounded font-medium",
                  element.is_container ? "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400"
                )}>
                  {String(element.is_container)}
                </span>
              </div>
            )}
            {element.has_direct_answer !== undefined && (
              <div className="flex items-center gap-2">
                <span className="text-gray-600 dark:text-gray-400">has_direct_answer:</span>
                <span className={cn(
                  "px-1.5 py-0.5 rounded font-medium",
                  element.has_direct_answer ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400"
                )}>
                  {String(element.has_direct_answer)}
                </span>
              </div>
            )}
          </>
        ...
        {displayConfig.element.showAnswerFormat && element.answer_format && (
          <div className="flex items-center gap-2">
            <span className="text-gray-600 dark:text-gray-400">answer_format:</span>
            <span className="text-gray-900 dark:text-gray-100 font-medium">{element.answer_format}</span>
          </div>
        )}

        {displayConfig.element.showAnswerRequirement && element.answer_requirement && (
          <div className="flex items-center gap-2">
            <span className="text-gray-600 dark:text-gray-400">answer_requirement:</span>
            <span className="text-gray-900 dark:text-gray-100 font-medium">{element.answer_requirement}</span>
          </div>
        )}
      </div>
    );
  ...
  const renderSubpart = (part: ComplexQuestionPartDisplay, subpart: ComplexQuestionSubpartDisplay, subpartIndex: number) => {
    if (!displayConfig.element.showSubparts) return null;
  ...
  const renderPart = (part: ComplexQuestionPartDisplay, partIndex: number) => {
    if (!displayConfig.element.showParts) return null;
      ...
      <div key={part.id} className="mb-6 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
        <div className="bg-gray-50 dark:bg-gray-900/50 p-4">
          ...
          >
            ...
            {/* Child count */}
            {displayConfig.hierarchy.containerShowsChildCount && hasSubparts && (
  ...
  return (
    <div className={cn('space-y-4', displayConfig.element.compactMode && 'text-sm')}>
      {/* Question header */}
      <div className="flex items-center justify-between pb-3 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-3">
          ...
          {displayConfig.element.showDifficulty && question.difficulty && (
            <span className={cn(
      ...
      {/* Main question text */}
      {displayConfig.element.showQuestionText && question.question_text && (
        <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
          ...
          {displayConfig.hierarchy.showContextualIndicator && (question.is_contextual_only || question.is_container) && (
      ...
      {/* Topic information */}
      {displayConfig.element.showTopic && (question.topic_name || question.subtopic_names) && (
      ...
      {/* Question-level hint */}
      {displayConfig.element.showHints && question.hint && (
        <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-700">
          <div className="flex items-start gap-2">
            <AlertCircle className="w-4 h-4 text-yellow-600 dark:text-yellow-400 mt-0.5" />
            <div>
      ...
      {displayConfig.element.showExplanations && question.explanation && (
        <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-700">
          <div className="flex items-start gap-2">
...
export default EnhancedComplexQuestionDisplay;
