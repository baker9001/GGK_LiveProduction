...
export const queryActiveStudents = async (
  ...
  try {
    ...
    return (data || []).map(student => ({
      ...student,
      name: student.name || student.users?.raw_user_meta_data?.name || student.users?.email?.split('@')[0] || 'Unknown Student',
...
export const queryActiveTeachers = async (
  ...
  try {
    ...
    return (data || []).map(teacher => ({
      ...teacher,
      name: teacher.name || teacher.users?.raw_user_meta_data?.name || teacher.users?.email?.split('@')[0] || 'Unknown Teacher',
